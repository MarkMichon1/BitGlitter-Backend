...
from bg_backend.bitglitter.utilities.gui.messages import write_render_http
...
def draw_frame(dict_obj):
    ...
    total_operations = dict_obj['total_operations']

    percentage_string = f'{round(((frame_number / total_operations) * 100), 2):.2f}'
    logging.info(f'Generating {frame_number} of {total_frames}... {percentage_string} %')
    write_render_http(frame_number, percentage_string)
    ...
    if save_statistics:
        from bg_backend.bitglitter.config.configfunctions import write_stats_update
